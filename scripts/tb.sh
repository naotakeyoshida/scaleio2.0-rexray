...
if [ -e "/home/vagrant/complete" ]; then
  ...
  if [ -e "/vagrant/jre-8u60-linux-x64.rpm" ]; then
      echo "`date +'%Y/%m/%d %H:%M:%S'` jre-8u60 was founded " >> $instlog
  else
  ...
  echo "`date +'%Y/%m/%d %H:%M:%S'` Java 1.8 installed " >> $instlog

  ## Install ScaleIO
  #cd /vagrant
  if [ -e "/vagrant/ScaleIO_Linux_v2.0.zip" ]; then
      echo "`date +'%Y/%m/%d %H:%M:%S'` ScaleIO_Linux_v2.0.zip is founded " >> $instlog
  else
      echo "`date +'%Y/%m/%d %H:%M:%S'` ScaleIO will be downloaded " >> $instlog
      ...
      echo "`date +'%Y/%m/%d %H:%M:%S'` Download complete " >> $instlog
  fi
  ...
  echo "`date +'%Y/%m/%d %H:%M:%S'` ScaleIO GW-IM installed " >> $instlog
  echo "================================================="
  ...
  echo "================================================="
  echo "`date +'%Y/%m/%d %H:%M:%S'`" > /home/vagrant/complete
fi
